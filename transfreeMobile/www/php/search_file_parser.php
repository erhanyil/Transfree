...
$pass = md5("NULL");
$sql = "SELECT * FROM tf_transfers WHERE transfer_email = '$yEmail' AND transfer_password = '$pass'";
...
if ($result->num_rows > 0) {
    ...
    while($row = $result->fetch_assoc()) {
			$downloadLink = "http://stdiosoft.com/transfree/test_uploads/".$row['transfer_email']."/".$row['transfer_file_name'];
			$kırpıkFileName = $row['transfer_file_name'];
			$kırpıkToEmail =  explode(",",$row['transfer_to_email']);
			$kk = array_pop($kırpıkToEmail);
			$countToEmail = count($kırpıkToEmail);
			$countToEmailTemp = 	$countToEmail;
			if($countForDisplay == 0){
			?>
				<div id="<?php echo $row['transfer_ID'] ?>" class="modal" name="searchDiv" style="display:block;margin-top:-20px">
		 <?php
			}else{
			?>
				<div id="<?php echo $row['transfer_ID'] ?>" class="modal" name="searchDiv"  style="display:none;margin-top:-20px">
			<?php
					}
			$countForDisplay++;
													...
													<div class="modal-content" style="border-radius:10px;">
														...
														<div class="modal-body" style="border-bottom:1px #ccc solid">
															<div class="alert alert-dismissible" style="padding-top:0px;padding-bottom:0px;padding-left:0px;margin-bottom:0px;margin-top:-10px">
																...
																<span id="fileSize" style="color:white" class="badge"><?php echo $row['transfer_file_size'] ?></span>
															</div>
														</div>
														<div class="modal-footer" style="text-align:left;padding:0;padding-bottom:15px">
															<div  style="margin-bottom:-10px" class="alert alert-dismissible">
																<p style="color:#000;word-wrap: break-word;" ><strong>Date: </strong><?php echo $row['transfer_time'] ?></p>
																<p style="color:#000;word-wrap: break-word;" ><strong>From: </strong><?php echo $row['transfer_email'] ?></p>
																<p style="color:#000;word-wrap: break-word;" ><strong>To: </strong><?php echo $kırpıkToEmail[0] ?></p>
																
<?php
				if($countToEmail > 1){
?>
																<a style="margin-left:2px;float:none;right:0px" onclick="showSearhtoEmailList(<?php echo $row["transfer_ID"] ?>)" class="close" > <span class="badge" style="background-color:#2196f3;">+<?php echo $countToEmail-1 ?> More Emails</span></a>
																	<ul class="list-group" id="toEmailList-<?php echo $row["transfer_ID"] ?>" style="display:none;padding:5px;overflow:auto;min-height:40px;max-height:50px;margin:0px;border:none;margin-top:5px;">
						<?php
